...
These functions clearly share a common underlying pattern.
...
We would like our language to be
powerful enough so that we can write a function that expresses the
concept of summation itself rather than only functions that compute
particular sums. We can do so readily by
taking the common template shown above and transforming the 'term' slot
...
```
(we cannot `recur` here as it is not in a tail position, trying to do so helpfully
throws `java.lang.UnsupportedOperationException: Can only recur from tail position`)
